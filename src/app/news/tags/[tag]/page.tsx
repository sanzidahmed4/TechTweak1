...
export async function generateMetadata({ params }: { params: Promise<{ tag: string }> }) {
  ...
  return {
    title: `${tag} News & Articles | TechTweak`,
    description: `Read all the latest smartphone news, reviews, and guides related to ${tag} on TechTweak.`,
  };
}

export default async function TagNewsPage({ params }: { params: Promise<{ tag: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-20">
      <div className="container mx-auto px-4 lg:px-8 mt-12">
        <div className="mb-12">
          <Link href="/news" className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-primary transition-colors mb-6">
            <ArrowLeft size={16} />
            Back to All News
          </Link>
          ...
          <p className="text-slate-500 mt-4 text-lg">
            {posts.length} {posts.length === 1 ? 'article' : 'articles'} found for this topic
          </p>
        </div>
